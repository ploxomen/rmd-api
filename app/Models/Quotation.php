...
class Quotation extends Model
{
    ...
    public static function getQuotations($search, $filters = [])
    {
        $query = Quotation::select("quotations.id", "quotation_code", "quotation_total", "quotation_type_money", "quotation_status", "customer_name")
            ->selectRaw("DATE_FORMAT(quotation_date_issue,'%d/%m/%Y') AS date_issue,CONCAT(user_name,' ',user_last_name) AS name_quoter,fn_name_subcategory_quotation(quotations.id) AS sub_categorie_name")
            ...
            ->where(function ($query) use ($search) {
                $query->where('customer_name', 'LIKE', '%' . $search . '%')
                    ...
                    ->orWhereRaw("fn_name_subcategory_quotation(quotations.id) LIKE CONCAT('%',?,'%')", [$search]);
            });
    ...
    public static function getQuotationsReport($startDate, $finalDate, $order = '0')
    {
        $quotationsQuery = Quotation::select("quotations.id", "quotation_code", "order_code", "quotation_date_issue", "customer_name", "user_name", "user_last_name", "contrie", "departament_name", "quotation_status", "quotation_type_money", "quotation_change_money")
        ...
        if ($order === '1') {
            return $quotationsQuery->whereNotNull('order_id');
        } else if ($order === '2') {
            return $quotationsQuery->whereNull('order_id');
        }
        return $quotationsQuery;
    }
}
